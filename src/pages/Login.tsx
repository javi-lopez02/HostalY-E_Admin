...
function Login() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    signIn({ username: inputUser.value, password: inputPassword.value }).then(
      () => {
        inputUser.value = "";
        inputPassword.value = "";
      }
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            {errors.map((err, index) => {
              return (
                <div
                  key={index}
                  className="rounded-md py-2 bg-red-500 flex items-center justify-center"
                >
                  <span className="text-white font-bold">{err}</span>
                </div>
              );
            })}
