...
export default function OfertsTable() {
  ...
  const filteredItems = useMemo(() => {
    ...
    let filteredProducts = [...oferts];

    if (hasSearchFilter) {
      filteredProducts = filteredProducts.filter((ofert) =>
        ofert.description.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    return filteredProducts;
  }, [oferts, hasSearchFilter, filterValue]);

  const pages = Math.ceil(filteredItems.length / rowsPerPage);

  const sortedItems = useMemo(() => {
    const sorted = [...filteredItems].sort((a: Oferts, b: Oferts) => {
